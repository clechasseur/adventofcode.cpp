...
void day20_1()
{
    ...
    //const std::string INPUT = "^ENNWSWW(NEWS|)SSSEEN(WNSE|)EE(SWEN|)NNN$";

    std::vector<room*> walkers{ start }, side;
    std::stack<std::vector<room*>> prev, prevside;
    for (size_t i = 0; i < INPUT.size(); ++i) {
        ...
        if (c == '^' || c == '$') {
            ...
            walkers.insert(walkers.end(), side.begin(), side.end());
            side = prevside.top();
            prevside.pop();
            prev.pop();
        } else if (c == '|') {
            side.insert(side.end(), walkers.begin(), walkers.end());
            walkers = prev.top();
        } else {
            for (room*& rpr : walkers) {
                rpr = area.move(rpr, c);
            }
        }
    }
