...
void day20_1()
{
    ...
    //const std::string INPUT = "(S(E|N)(S|W)W)";

    struct re_node {
        re_node* pnext = nullptr;
        virtual ~re_node() = default;

        struct visitor {
            virtual ~visitor() = default;
            virtual void move(char m) = 0;
            virtual void end_move() = 0;
            virtual std::shared_ptr<visitor> clone() const = 0;
        };
        virtual std::vector<std::shared_ptr<visitor>> visit_this(const std::shared_ptr<visitor>& spvisitor) const = 0;
        void visit_multiple(std::vector<std::shared_ptr<visitor>>& visitors) const {
            std::vector<std::shared_ptr<visitor>> newvisitors;
            for (auto&& spvisitor : visitors) {
                auto newvisitorsthistime = visit_this(spvisitor);
                newvisitors.insert(newvisitors.end(), newvisitorsthistime.begin(), newvisitorsthistime.end());
            }
            visitors.insert(visitors.end(), newvisitors.begin(), newvisitors.end());
        }
        std::vector<std::shared_ptr<visitor>> visit(const std::shared_ptr<visitor>& spvisitor, bool is_top = true) const {
            std::vector<std::shared_ptr<visitor>> visitors{ spvisitor };
            const re_node* pcur = this;
            while (pcur != nullptr) {
                pcur->visit_multiple(visitors);
                pcur = pcur->pnext;
            }
            if (is_top) {
                for (auto&& spv : visitors) {
                    spv->end_move();
                }
            }
    ...
    struct re_l_node : re_node {
        char l = 0;
        virtual std::vector<std::shared_ptr<visitor>> visit_this(const std::shared_ptr<visitor>& spvisitor) const override {
            spvisitor->move(l);
            return { };
        }
    };
    struct re_sub_node : re_node {
        std::vector<re_node*> sub;
        virtual std::vector<std::shared_ptr<visitor>> visit_this(const std::shared_ptr<visitor>& spvisitor) const override {
            std::vector<std::shared_ptr<visitor>> newvisitors, newvisitorsafter;
            if (!sub.empty()) {
                newvisitors.reserve(sub.size() - 1);
                for (size_t i = 0; i < (sub.size() - 1); ++i) {
                    newvisitors.emplace_back(spvisitor->clone());
                }
                size_t newidx = 0;
                for (auto it = sub.begin(); it != sub.end(); ++it) {
                    auto spthisvisitor = it == sub.begin() ? spvisitor
                                                           : newvisitors[newidx++];
                    auto newsubvisitors = (**it).visit(spthisvisitor, false);
                    newvisitorsafter.insert(newvisitorsafter.end(), newsubvisitors.begin(), newsubvisitors.end());
                }
            }
            newvisitors.insert(newvisitors.end(), newvisitorsafter.begin(), newvisitorsafter.end());
            return newvisitors;
        }
    };
    struct re_empty_node : re_node {
        virtual std::vector<std::shared_ptr<visitor>> visit_this(const std::shared_ptr<visitor>& spvisitor) const override {
            return { };
        }
    };
    std::vector<std::shared_ptr<re_node>> re_node_storage;
    std::function<re_node*(std::string::const_iterator&)> parse_subtree = [&](std::string::const_iterator& it) -> re_node* {
        auto spnode = std::make_shared<re_sub_node>();
        re_node_storage.push_back(spnode);
        auto spfirstsub = std::make_shared<re_empty_node>();
        re_node_storage.push_back(spfirstsub);
        re_node* pcursub = spfirstsub.get();
        spnode->sub.push_back(pcursub);

        bool done = false;
        while (!done) {
            switch (*it) {
                case '(': {
                    pcursub->pnext = parse_subtree(++it);
                ...
                case '|': {
                    auto spnewsub = std::make_shared<re_empty_node>();
                    re_node_storage.push_back(spnewsub);
                    pcursub = spnewsub.get();
                    spnode->sub.push_back(pcursub);
                    ++it;
                    break;
                }
                default: {
                    auto splnode = std::make_shared<re_l_node>();
                    re_node_storage.push_back(splnode);
                    splnode->l = *it;
                    pcursub->pnext = splnode.get();
                    pcursub = pcursub->pnext;
                    ++it;
                    break;
                }
    ...
    auto re_it = INPUT.cbegin() + 1;
    re_node* ptree = parse_subtree(re_it);

    struct test_visitor : re_node::visitor {
        size_t* ptotal;
//        std::string s;
        test_visitor(size_t* pt) : ptotal(pt) { }
...
//            std::cout << s << std::endl;
        ...
        virtual std::shared_ptr<visitor> clone() const {
            return std::make_shared<test_visitor>(*this);
        }
    };
    size_t total = 0;
    auto spvisitor = std::make_shared<test_visitor>(&total);
    ptree->visit(spvisitor);
    std::cout << total << " possible paths." << std::endl;
}
